...
class Gui(customtkinter.CTk):
    def __init__(self):
        ...
        self.window_with_file = customtkinter.CTkEntry(
                master=self.chance_file, width=400, placeholder_text='Введите путь к файлу', corner_radius=0
        )
        self.window_with_file.grid(row=0, column=0, sticky='e')

        self.clipboard = customtkinter.CTkButton(
                master=self.chance_file, text='Вставить', command=self.btn_clip, width=60, corner_radius=0
        )
        self.clipboard.grid(row=0, column=0, sticky='e')

        self.btn_open = customtkinter.CTkButton(
                master=self.chance_file, text='Открыть файл', command=self.open_file, corner_radius=0
        )
        self.btn_open.grid(row=0, column=1, padx=(15, 0))

        # настройки
        self.chance_mode1 = customtkinter.CTkRadioButton(
                self.setting_frame, variable=self.mode, value=1, text='Авто-локализация', state='disabled'
        )
        self.chance_mode1.grid(row=1, column=0, padx=10, pady=(10, 5), sticky='w')
        self.chance_mode2 = customtkinter.CTkRadioButton(
                self.setting_frame, variable=self.mode, value=2, text='Сделать заготовку', state='disabled'
        )
