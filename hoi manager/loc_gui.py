...
class Gui(customtkinter.CTk):
    def __init__(self):
        ...
        self.window_with_file = customtkinter.CTkEntry(master=self, width=400, placeholder_text='Введите путь к файлу', corner_radius=0)
        self.window_with_file.grid(row=0, column=0, padx=(5, 0), pady=(20, 0), sticky='e')

        self.clipboard = customtkinter.CTkButton(master=self, text='Вставить', command=self.btn_clip, width=60, corner_radius=0)
        self.clipboard.grid(row=0, column=0, padx=0, pady=(20, 0), sticky='e')

        self.btn_open = customtkinter.CTkButton(master=self, text='Открыть файл', command=self.open_file, corner_radius=0)
        ...
        self.check_zero = customtkinter.CTkCheckBox(
                self.setting_frame, text='Вставлять ли "0"?', variable=self.need_zero, onvalue='да', offvalue='нет', state='disabled'
        )
        ...
        self.check_space = customtkinter.CTkCheckBox(
                self.setting_frame, text='Вставлять ли пробелы?', variable=self.need_space, onvalue='да', offvalue='нет', state='disabled'
        )
